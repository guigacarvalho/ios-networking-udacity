...
class LoginViewController: UIViewController {
    ...
    func loginWithToken(requestToken: String) {
        ...
        let task = session.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard let success = parsedResult["success"] as? Bool else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Login Failed (Login Failed)."
                }
                print("Cannot find key 'success' in \(parsedResult)")
                return
            }
    ...
    func getSessionID(requestToken: String) {
        ...
        let task = session.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard (error == nil) else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Login Failed (getting SessionID)."
                }
                print("There was an error with your request: \(error)")
                return
            }
