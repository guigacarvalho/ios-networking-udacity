...
class LoginViewController: UIViewController {
    ...
    func getUserID(session_id : String) {
        ...
        let task = session.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard (error == nil) else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Failed getting user ID."
                }
                print("There was an error with your request: \(error)")
                return
            }
            ...
            /* GUARD: Is the "userID" key in parsedResult? */
            guard let userID = parsedResult["id"] as? Int else {
                dispatch_async(dispatch_get_main_queue()) {
                    self.debugTextLabel.text = "Failed getting user ID in parsedResults."
                }
                print("Cannot find key 'userID' in \(parsedResult)")
                return
            }
